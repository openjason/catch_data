...
class App:
    ...
    def dingding_data_ech(self, xlsfilename):
        ...
        try:
            ...
            if 'Unsupported format, or corrupt file: Expected BOF record' in err_message_str or \
            'not a zip file' in err_message_str:
        ...
        userid_count = 0
        userid_attend = 0
        sheet_title = sheet_curr.cell(0, 0).value
        self.scr.insert(
            END, "\nExcel File Title: " + sheet_title + "...\n"
        )
        sheet_title_lastday = sheet_title[-10:]
        if str_kaoqin_date_begin > sheet_title_lastday:
            self.scr.insert(
                END, "\n错误：处理考勤数据日期超出Excel表格日期范围， 请确认后重试，File Title: " + sheet_title + "...\n"
            )
            return(-60)
        ...
        self.scr.insert(END, "..\n")
        self.scr.insert(END, "导出数据完成，保存文件名 nc.txt    \n")
