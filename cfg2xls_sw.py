...
def fix_netmask(add_str):
    netmask_str = add_str.replace(' 255.255.255.0','/24')
    return netmask_str
    ...
    for i in range(len(blocked_list)):
        ...
        if 'address-object' == tempStr1[:14]:
            if aname == tempStr1[20:20+len(aname)] and tempStr1[20+len(aname):21+len(aname)]== ' ':
                try:
                    zone_pos = tempStr1.index('zone')
                except:
                    zone_pos = len(tempStr1)
                address_detail = tempStr1[21 + len(aname):zone_pos]
                break
    if address_detail[:4] == 'host':
        address_detail = address_detail[5:]
    if address_detail[:7] == 'network':
        address_detail = fix_netmask(address_detail)
    if address_detail[:5] == 'range':
        try:
            first_space = address_detail[10:len(address_detail)].index(' ')
        except:
            first_space = 0
        address_detail = address_detail[:10+first_space] + '-' + address_detail[first_space+11:len(address_detail)]
