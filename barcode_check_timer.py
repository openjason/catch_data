...
class Watch(Frame):
    ...
    def _settime(self):
        ...
        print('str_test_catch_ch',str(sampling_entry_context_len),len(str_test_catch_ch),time_delta)
...
# 给窗口的可视化起名字
window.title('二维码内容检测匹配工具 v.19081910')
...
def processKeyboardEvent(ke):
    if ke.keycode < 20:
        return
    if ke.keycode > 128:
        #print('up128',ke.keycode,ke.char)
        return
    ...
    time_barcode_read_time = time.time()
    #print(ke.keycode,ke.char)
    
    str_test_catch_ch = str_test_catch_ch + ke.char

    #print('checkString: ',str_test_catch_ch)
