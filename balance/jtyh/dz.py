...
class App():
    def __init__(self, master):
        ...
        self.svar_youjiqingdan_filename1 = StringVar()
        self.svar_youjiqingdan_filename2 = StringVar()
        self.svar_youjiqingdan_filename3 = StringVar()
        self.svar_youjiqingdan_filename4 = StringVar()
        ...
        self.file_from_youjiqingdan1 = ''
        self.file_from_youjiqingdan2 = ''
        self.file_from_youjiqingdan3 = ''
        self.file_from_youjiqingdan4 = ''
...
where kehu='" + customer + "' and wuliao = '" + str(curr_wuliao[0]) + "'"
                ...
                for row in sqlcursor:
                    if row[0] > 0:
                        # print("fachu = ", row[0])
                        list_return_from_db.append([curr_wuliao[1], row[0],row[1],row[2],row[3]])
...
# 获取空白卡数量（物料，发出数，按月计算）
        ...
        for i in range(len(list_kongbaika)):
            ...
            #worksheel.cell(int_first_row + i, 5).value = list_kongbaika[i][1]          不需要发出数，发出数是合计，现需要细项
            worksheel.cell(int_first_row + i, 5).value = list_kongbaika[i][2]           #成品
            worksheel.cell(int_first_row + i, 6).value = list_kongbaika[i][4]           #剪卡/取消卡
            worksheel.cell(int_first_row + i, 7).value = list_kongbaika[i][3]           #废卡
            worksheel.cell(int_first_row + i, 8).value = 0                              #现无样卡数据来源，置0，手工处理
            ...
            for j in range(len(list_price_mingcheng)):
                ...
                if list_kongbaika_mingcheng in list_price_mingcheng[j]:
                    worksheel.cell(int_first_row + i, 2).value = list_price[j][1]           #版本号
                    worksheel.cell(int_first_row + i, 4).value = list_price[j][6]           #单价
                    worksheel.cell(int_first_row + i, 9).value = list_kongbaika[i][1] * list_price[j][6]    #合计
        ...
        worksheel.cell(int_first_row + i + 1, 7).value = '=SUM(G3:G' + str(int_first_row + i) + ')'
        worksheel.cell(int_first_row + i + 1, 8).value = '=SUM(H3:H' + str(int_first_row + i) + ')'
        worksheel.cell(int_first_row + i + 1, 9).value = '=SUM(I3:I' + str(int_first_row + i) + ')'
    ...
    def xls_db(self, customer, xlsfilename):
        ...
        for i in range(int_first_row, int_sheet_nrows):
            ...
            if not isinstance(cell_curr_value, str):  # 判断数据是否最后一行
                ...
                print('lines: cell value: ',i,cell_curr_value )
...
values("' + customer + '","' + wuliao + '","' + str_date + '","' + str(fachu) + '",' + str(chengpin) + ',' + str(
    ...
    def mailxls_db(self, customer, xlsfilename):
        ...
        temp_file_list = self.find_filename("..\\仓库文件\\", self.file_from_youjiqingdan)
        ...
        if len(temp_file_list)>4:
            logging.info('无法处理多于 4个邮寄清单文件。。。')
            return_message = messagebox.askquestion(title='提示',message='无法处理多余4个邮寄清单文件。继续 ？') #return yes/no
            return (return_message)
        ...
        for xlsfilename in temp_file_list:
            workbook = xlrd.open_workbook(xlsfilename)
            sheetsname = workbook.sheet_names()  # 获取excel里的工作表sheet名称数组
            ...
            for i in range(int_first_row, int_sheet_nrows):
                ...
                if i % 100 == 0:
                    ...
                    self.scr.insert(1.0, "数据导入  邮寄清单（maillist）"+str(i) +"条.\n")
                    self.master.update()
                ...
                if True:  # not isinstance(cell_curr_value,str):         #判断数据是否最后一行
                    ...
                    if int(maillistid) > 0:  # testing
                        # 插入数据
                        str_sql = "insert into maillist(kehu,picihao,shenqinbianhao,youjifangshi,youjidanghao,jichudi,zhikafangshi,\
    ...
    emsliushuihao,pid,quyu)"
                        str_sql = str_sql + "values('" + customer + "','" + picihao + "','" + shenqinbianhao + "'," + \
                        ...
                        # 如果隔离级别不是自动提交就需要手动执行commit
                        self.sqlconn.commit()
            print(xlsfilename + 'procesed: ', i, 'record.')
            self.scr.insert(1.0, '数据导入..' + xlsfilename + "邮寄清单（maillist）" + str(i) + ".\n")
            self.master.update()
...
fuwuleixingbiaoshi + "','" + xinpianka + "','" + gongjiqueren + "')"
                    ...
                    self.scr.insert(1.0, "数据导入...基础数据表（price）数据导入: " + str(kapianmingcheng) + ".\n")
                    self.master.update()
        ...
        self.scr.insert(1.0, "数据导入..基础数据表（price）"+str(i - int_first_row + 1)+"行数据..\n")
        self.master.update()
    ...
    def initWidgets(self,fm1):
        ...
        temp_file_list = self.find_filename("..\\仓库文件\\", self.file_from_cangkujxc)
        if len(temp_file_list)>0:
            self.file_from_cangkujxc = temp_file_list[0]
        temp_file_list = self.find_filename("..\\仓库文件\\", self.file_from_youjiqingdan)
        if len(temp_file_list)>4:
            logging.info('无法处理多余4个邮寄清单文件。。。')
        elif len(temp_file_list) > 3:
            self.file_from_youjiqingdan1 = temp_file_list[0]
            self.file_from_youjiqingdan2 = temp_file_list[1]
            self.file_from_youjiqingdan3 = temp_file_list[2]
            self.file_from_youjiqingdan4 = temp_file_list[3]
        elif len(temp_file_list) > 2:
            self.file_from_youjiqingdan1 = temp_file_list[0]
            self.file_from_youjiqingdan2 = temp_file_list[1]
            self.file_from_youjiqingdan3 = temp_file_list[2]
        elif len(temp_file_list) > 1:
            self.file_from_youjiqingdan1 = temp_file_list[0]
            self.file_from_youjiqingdan2 = temp_file_list[1]
...
#        entry_proc_time2.place(x=440, y=55)
        ...
        label_jichu_filename.place(x=620, y=30)
        ...
        entry_jichu_filename.place(x=620, y=55)

        label_cangku_filename = Label(fm1, text='仓库进销存文件名：', font=('Arial', 12))
        label_cangku_filename.place(x=620, y=90)
        ...
        entry_cangku_filename.place(x=620, y=115)

        label_youjiqingdan_filename = Label(fm1, text='邮寄清单文件名：', font=('Arial', 12))
        label_youjiqingdan_filename.place(x=620, y=150)
        self.svar_youjiqingdan_filename1.set(self.file_from_youjiqingdan1)
        self.svar_youjiqingdan_filename2.set(self.file_from_youjiqingdan2)
        self.svar_youjiqingdan_filename3.set(self.file_from_youjiqingdan3)
        self.svar_youjiqingdan_filename4.set(self.file_from_youjiqingdan4)


        entry_youjiqingdan_filename1 = Entry(fm1, textvariable=self.svar_youjiqingdan_filename1, width=40, font=('Arial', 12))
        entry_youjiqingdan_filename1.place(x=620, y=185)
        entry_youjiqingdan_filename2 = Entry(fm1, textvariable=self.svar_youjiqingdan_filename2, width=40, font=('Arial', 12))
        entry_youjiqingdan_filename2.place(x=620, y=215)

        entry_youjiqingdan_filename3 = Entry(fm1, textvariable=self.svar_youjiqingdan_filename3, width=40, font=('Arial', 12))
        entry_youjiqingdan_filename3.place(x=620, y=245)
        entry_youjiqingdan_filename4 = Entry(fm1, textvariable=self.svar_youjiqingdan_filename4, width=40, font=('Arial', 12))
        entry_youjiqingdan_filename4.place(x=620, y=275)
...
#        btn_barcode_check.place(x=650, y=160)
        ...
        btn_barcode_init.place(x=620, y=400)
...
if __name__ == '__main__':
    ...
    set_logging()
    main_window = Tk()
    main_window.title('对账单生成工具 v.19090414')
